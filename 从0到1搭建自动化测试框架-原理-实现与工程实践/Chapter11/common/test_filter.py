...
class TestFilter:
    ...
    def filter_tags_in_all(self, user_option_tags):

        included_cls = []
        remain_cases = []

        for i in self.suites:
            tags_in_class = getattr(i[0], 'tags', None)
            tags = []
            ...
            if all(map(lambda x: True if x in after_parse else False, user_option_tags)):
                included_cls.append(i[0])
